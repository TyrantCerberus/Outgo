...
/datum/preferences/proc/SetChoices(mob/user, limit = 15, widthPerColumn = 295, height = 620)
	...
	if(length(SSjob.joinable_occupations) <= 0)
		...
		for(var/datum/job/job as anything in sort_list(SSjob.joinable_occupations, /proc/cmp_job_display_asc))
			...
			var/rank = job.title
			var/rank_name = "[job.trainee_role ? "[rank] (Trainee Role)" : "[rank]"]"
			lastJob = job
			...
			if(required_playtime_remaining)
				HTML += "<font color=red>[rank_name]</font></td><td><font color=red> \[ [get_exp_format(required_playtime_remaining)] as [job.get_exp_req_type()] \] </font></td></tr>"
				continue
			if(!job.player_old_enough(user.client))
				var/available_in_days = job.available_in_days(user.client)
				HTML += "<font color=red>[rank_name]</font></td><td><font color=red> \[IN [(available_in_days)] DAYS\]</font></td></tr>"
				continue
