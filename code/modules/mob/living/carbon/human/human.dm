	...
	if(href_list["hud"])
		...
		if(href_list["hud"] == "m")
			...
			if(!H.wear_id) //You require access from here on out.
				...
				return
			var/list/access = H.wear_id.get_access()
...
/mob/living/carbon/human/assess_threat(judgement_criteria, lasercolor = "", datum/callback/weaponcheck=null)
	...
	if( (judgement_criteria & JUDGE_WEAPONCHECK) && weaponcheck)
		if(!idcard || !(ACCESS_WEAPONS in idcard.get_access()))
...
/mob/living/carbon/human/update_hud_pain_meter(var/pain_slowdown_level)
	var/hud_element_to_modify = src.hud_used.infodisplay.Find(/atom/movable/screen/pain)
