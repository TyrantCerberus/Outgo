...
/datum/species/proc/grab(mob/living/carbon/human/user, mob/living/carbon/human/target, datum/martial_art/attacker_style)
	...
	if(attacker_style?.grab_act(user,target) == MARTIAL_ATTACK_SUCCESS)
		...
		//Steal them shoes
		if(target.body_position == LYING_DOWN && (user.zone_selected == BODY_ZONE_L_LEG || user.zone_selected == BODY_ZONE_R_LEG) \
		&& user.istate.control && target.shoes && !(target?.w_uniform?.body_parts_covered & FEET || target?.wear_suit?.body_parts_covered & FEET))
...
/datum/species/proc/apply_damage(damage, damagetype = BRUTE, def_zone = null, blocked, mob/living/carbon/human/H, forced = FALSE, spread_damage = FALSE, sharpness = NONE)
	SEND_SIGNAL(H, COMSIG_MOB_APPLY_DAMGE, damage, damagetype, def_zone, sharpness)
...
/datum/species/proc/handle_environment(mob/living/carbon/human/humi, datum/gas_mixture/environment, delta_time, times_fired)
	return handle_environment_pressure(humi, environment, delta_time, times_fired)
...
/datum/species/proc/handle_body_temperature(mob/living/carbon/human/humi, delta_time, times_fired)
	...
	if(!IS_IN_STASIS(humi))
		return body_temperature_damage(humi, delta_time, times_fired)
...
/datum/species/proc/body_temperature_alerts(mob/living/carbon/human/humi)
	...
	if(humi.bodytemperature > bodytemp_heat_damage_limit && !HAS_TRAIT(humi, TRAIT_RESISTHEAT))
		...
		if(bodytemp in BODYTEMP_HEAT_WARNING_2 to BODYTEMP_HEAT_WARNING_3)
	...
	// Body temperature is too cold, and we do not have resist traits
		...
		if(bodytemp in BODYTEMP_COLD_WARNING_3 to BODYTEMP_COLD_WARNING_2)
...
/datum/species/proc/body_temperature_damage(mob/living/carbon/human/humi, delta_time, times_fired)
	...
	if(humi.coretemperature > bodytemp_heat_damage_limit && !HAS_TRAIT(humi, TRAIT_RESISTHEAT))
		...
		humi.adjustFireLoss(burn_damage)
	...
	if(humi.coretemperature < bodytemp_cold_damage_limit && !HAS_TRAIT(humi, TRAIT_RESISTCOLD))
		...
		if(humi.coretemperature in 201 to bodytemp_cold_damage_limit)
			humi.adjustFireLoss(COLD_DAMAGE_LEVEL_1 * damage_mod * delta_time)
		else if(humi.coretemperature in 120 to 200)
			humi.adjustFireLoss(COLD_DAMAGE_LEVEL_2 * damage_mod * delta_time)
		else
			humi.adjustFireLoss(COLD_DAMAGE_LEVEL_3 * damage_mod * delta_time)
		return TRUE
