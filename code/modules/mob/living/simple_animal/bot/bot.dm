...
/mob/living/simple_animal/bot/attackby(obj/item/W, mob/living/user, params)
	if(W.tool_behaviour == TOOL_SCREWDRIVER)
		...
		if(W.tool_behaviour == TOOL_WELDER && !user.istate.harm)
			if(health >= maxHealth)
...
/mob/living/simple_animal/bot/proc/bot_move(dest, move_speed)
	...
	if(step_count >= 1 && tries < BOT_STEP_MAX_RETRIES)
		for(var/step_number = 0, step_number < step_count,step_number++)
			addtimer(CALLBACK(src, PROC_REF(bot_step)), BOT_STEP_DELAY*step_number)
	else
		return FALSE
	return TRUE
...
/mob/living/simple_animal/bot/proc/call_bot(caller, turf/waypoint, message=TRUE)
	...
	var/obj/item/card/id/all_access = new /obj/item/card/id/tier5/captains_spare()
	...
	if(path?.len) //Ensures that a valid path is calculated!
		...
		access_card.set_access(REGION_ACCESS_STATION) //Give the bot all-access while under the AI's command.
		if(client)
