...
/obj/machinery/computer/arcade/orion_trail/proc/report_player(mob/gamer)
	...
	if(gamers[gamer] > ORION_GAMER_REPORT_THRESHOLD && prob(20 * gamers[gamer]))
		...
		gamer.mind?.adjust_experience(/datum/skill/gaming, 100) // cheevos make u better
	...
	switch(action)
		...
		if("continue")
			if(turns >= ORION_TRAIL_WINTURN)
				...
				xp_gained += 34
				return
			gamer.mind.adjust_experience(/datum/skill/gaming, xp_gained+2)
		...
		if("buycrew") //buy a crewmember
			if(!spaceport_raided && food >= 10 && fuel >= 10 && gameStatus == ORION_STATUS_MARKET)
				if(params["odd"])
					...
					add_crewmember(pick(GLOB.commando_names + GLOB.nightmare_names + GLOB.ai_names + GLOB.clown_names + GLOB.mime_names + GLOB.plasmaman_names + GLOB.carp_names))
				else
					add_crewmember()
...
/obj/machinery/computer/arcade/orion_trail/proc/death_reason(mob/living/carbon/gamer)
	...
	gamer?.mind?.adjust_experience(/datum/skill/gaming, 20)//learning from your mistakes is the first rule of roguelikes
	return reason
